...
function registerUser() {
  ...
  let cpw = document.forms["registerForm"]["cpw"].value;
  if (document.getElementById("chkAgree").checked == true) {
    if(fname.length==0 || email.length==0 || pw.length==0 || cpw.length ==0) {
...
function loginUser() {
  ...
  let pw = document.forms["loginForm"]["lpw"].value;
  if(email.length>0 && pw.length>0) {
    let users = localStorage.getItem('users');
  if(users) {
    users = JSON.parse(users);
    var result = users.filter(obj => {
      return obj.email == email && obj.pw == pw
    });
    console.log(result);
    if(result.length>0) {
